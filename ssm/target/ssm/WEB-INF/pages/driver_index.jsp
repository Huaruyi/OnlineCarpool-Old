			...
			<div class="tabbable" id="tabs-592780">
				<ul class="nav nav-tabs">
					...
					<li>
						...
						<script>
                            function TobookList() {
                                window.location.href="driverBookList";
                            }
						</script>
					</li>
				</ul>
						...
						<div class="row-fluid">
							<div class="span12">
								<table class="table table-hover table-bordered" contenteditable="false">
									...
									<tbody>
									<c:forEach items="${driverPublishList}" var="pl">
										<tr class="success">
											<td>${pl.bid}</td>
											<td>${pl.route.rdeparture} &nbsp;→&nbsp; ${pl.route.rdestination}</td>
											<td>
												<c:if test="${not empty pl.did}">
													...
													<div class="modal-dialog" role="document">
														<div class="modal-content">
															...
															<div class="modal-body">
																<form id="addPublishForm" action="driverAddPublish" method="post">
                                                                    <div class="form-group opt">
                                                                        <div class="col-sm-6">
                                                                            <input type="hidden" name="did" id="did" value="${driver.did}"><br>
                                                                        </div>
                                                                    </div>
                                                                    <div class="form-group opt">
                                                                        <div class="col-sm-6">
												...
												<div class="modal fade" id="detailModal" tabindex="-1" role="dialog" aria-labelledby="detailModalLabel">
													<div class="modal-dialog" role="document">
														<div class="modal-content">
															...
															<div class="modal-body">
																<h4>乘客信息</h4>
																<%--<c:if test="${empty pl.bookdetail1}">
																	乘客还未加入
																</c:if>

																	<c:forEach items="${pl.bookdetail1.user}" var="usr">
																		姓名：${usr.uname}<br>
																		电话：${usr.uphone}<br>
																		拼车：${usr.utimes}次<br>
																		评价：${usr.uevaluate}分<br>
																	</c:forEach>--%>
										...
										<nav aria-label="Page navigation">
											<ul class="pagination">
												...
												<c:if test="${alldriverPublishPage.pageNum != alldriverPublishPage.firstPage}">
													<li>
														<a href="driverPublishList?page=${alldriverPublishPage.firstPage}"><span aria-hidden="true">首页</span></a>
													</li>
												</c:if>
												<c:if test="${alldriverPublishPage.pageNum == alldriverPublishPage.firstPage}">
													<li  class="disabled">
												...
												<c:if test="${alldriverPublishPage.hasPreviousPage}">
													<li>
														<a href="driverPublishList?page=${alldriverPublishPage.pageNum-1}" aria-label="Previous"><span aria-hidden="true">&laquo;</span></a>
													</li>
												</c:if>
												<!-- 页码  -->
												<!-- 当总页数小于等于7时，显示页码1...7页 -->
												<c:if test="${alldriverPublishPage.pages<=7}">
													<c:forEach begin="1" end="${alldriverPublishPage.pages}" var="i">
														<li <c:if test="${alldriverPublishPage.pageNum==i }">class="active"</c:if>>
															<a href="driverPublishList?page=${i}">${i}</a>
														</li>
													</c:forEach>
												</c:if>
												<!-- 当总页数大于7时 -->
												<c:if test="${alldriverPublishPage.pages>7}">
													...
													<c:if test="${alldriverPublishPage.pageNum<=4}">
														...
														<li
															...
															<a href="driverPublishList?page=${alldriverPublishPage.pages}">${alldriverPublishPage.pages}</a>
														</li>
													</c:if>
													...
													<c:if test="${alldriverPublishPage.pageNum>4}">
														<c:if test="${alldriverPublishPage.pageNum<alldriverPublishPage.pages-3}">
															<li>
																<a href="driverPublishList?page=${1}">${1}</a>
															</li>
															<li><a href="#">...</a></li>
													...
													<c:if test="${alldriverPublishPage.pageNum>alldriverPublishPage.pages-4}">
														<li><a href="driverPublishList?page=${1}">${1}</a>
														</li>
												...
												<c:if test="${alldriverPublishPage.hasNextPage}">
													<li>
														<a href="driverPublishList?page=${alldriverPublishPage.pageNum+1}" aria-label="Next"><span aria-hidden="true">&raquo;</span></a>
													</li>
												</c:if>
												<%--尾页--%>
												<c:if test="${alldriverPublishPage.pageNum != alldriverPublishPage.lastPage}">
													<li>
														<a href="driverPublishList?page=${alldriverPublishPage.lastPage}"><span aria-hidden="true">尾页</span></a>
													</li>
												</c:if>
												<c:if test="${alldriverPublishPage.pageNum == alldriverPublishPage.lastPage}">
													<li  class="disabled">
